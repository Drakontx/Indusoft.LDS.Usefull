...
namespace Indusoft.LDS.Usefull
{
    ...
    public static partial class g
    {
        ...
        public static void WriteBounds(double Low, byte LowDigits, double High, byte HighDigits, params AnalogTechTest[] techTests)
        {
            ...
            foreach (AnalogTechTest techTest in techTests)
            {
                if (techTest.Exists)
                {
                    ...
                    {
                        techTest.SetLessOrEqual();
                        techTest.FValue = String.Format("менее {0}", Low.ToString(lowFormat));
                    }

                    if (Low <= techTest.Value && techTest.Value <= High)
                    {
                        techTest.SetPrefix(ValuePrefix2.None);
                    }

                    if (High < techTest.Value)
                    {
                        techTest.SetMoreOrEqual();
        ...
        /// <param name="HighDigits">Количество знаков после запятой для верхней границы</param>
        /// <param name="techTests">Показатели, для которых проверяется граница</param>
        public static void WriteBounds(ValuePrefix2 LowPrefix, double Low, int LowDigits, ValuePrefix2 HighPrefix, double High, int HighDigits, params AnalogTechTest[] techTests)
        {
            string lowFormat = LowDigits == 0 ? "" : ".";
            for (int i = 0; i < LowDigits; i++) { lowFormat += "0"; }
